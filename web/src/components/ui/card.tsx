...
/** Base surface card. Prefer `GlowCard` from common/ for page sections that should lift on hover. */
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn("surface-card flex flex-col gap-5 py-5 text-card-foreground", className)}
      {...props}
    />
  );
}
...
function CardTitle({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("font-display text-base leading-tight font-semibold text-text", className)}
      {...props}
    />
  );
}
